...
void logOpen()
{
	...
	syslog(LOG_ERR,"Ошибка открытия журнала (%s): %s",config.log_filename,strerror(errno));
    ...
    else if(control.debug_level>DEBUG_LEVEL_NORMAL)
    {
	//sprintf(msg,"Журнал (%s)успешн открыт для записи",config.log_filename);
	syslog(LOG_NOTICE,"Журнал (%s) успешн открыт для записи",config.log_filename);
    }
}
